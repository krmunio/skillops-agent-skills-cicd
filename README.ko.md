...
**샘플 프로젝트 → 샘플 화면 보기**에서 project-a·b 각각 개선·유지·악화 3회분을 확인할 수 있습니다.
각 Skill의 합성 원본·후보, 생성 이유·diff, 기본 품질·프로젝트 재평가 예시를 포함합니다.
이름·경로만 실제 탐지 정보이고 본문·수치·시각은 합성입니다. 샘플은 실제 이력과 분리되며,
모든 후보는 미채택 상태입니다. 유료 호출이나 원본 변경 없이 정적 빌드에서 생성합니다.
