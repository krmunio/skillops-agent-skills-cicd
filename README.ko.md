...
| GEPA 통합 | 공식 GEPA 옵티마이저의 Pareto 부모 선택을 선택적으로 사용하고 후보군·검사별 점수를 보존합니다. 별도 최종 확인과 승인 게이트는 유지합니다. |
...
기본 경로는 APO-inspired 개선 근거와 순차 탐색을 유지합니다.
선택적인 `iterate --optimizer gepa` 경로는 공식 GEPA 옵티마이저와 Pareto 기반 부모 선택을
연결합니다. 하나의 기록된 개발 작업에 포함된 필수 검사들을 기준으로 탐색하고 후보 계보와
점수를 보존합니다. 개발 검사들은 탐색 검증에도 재사용하며, 독립된 최종 확인 근거로
표현하지 않습니다. 최종 확인·사람 승인·버전 사용·게시 사이의 통제 경계는 유지합니다.
설치·점수·한도·버전별 근거 계약은 [GEPA 통합 안내](docs/GEPA.md)를 참고하세요.
...
선택적인 GEPA 탐색에는 해시로 고정한 core 패키지만 설치합니다. 외부 모델 제공자용 extras는
설치하지 않습니다.
...
기본값은 `--optimizer sequential`입니다. `--optimizer gepa`는 필수 검사 2–128개가 있는
기록된 개발 WorkItem과 기존의 명시적인 live 권한·공유 실행 한도를 요구합니다.
탐색 완료는 개선 입증·최종 확인·승인·배포를 뜻하지 않습니다.
