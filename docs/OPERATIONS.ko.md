# SkillOps 상세 운영 가이드와 과거 평가 근거

[제품 개요](../README.ko.md) | [English guide](OPERATIONS.md)

2026년 9월 21일 README 개편 시 보존한 상세 명령, 안전 계약과 과거 측정 기록입니다.
명령은 `docs/`가 아니라 저장소 루트에서 실행합니다. 과거 측정값은 현재 코드나
평가기의 검증 근거가 아닙니다. 현재 제품 소개는 위의 제품 개요를 참고하세요.
...
대시보드는 브라우저 언어와 무관하게 영어로 시작합니다. **Language / 언어**
선택기에서 **English / 한국어**를 선택하면 다음 방문에도 유지됩니다.
브라우저 저장소를 사용할 수 없어도 이번 방문에서 언어를 전환할 수 있으며,
저장 불가 안내를 표시합니다. 언어 전환은 프로젝트·실행·Skill·이력 탭·샘플
선택을 유지하며 평가 조회나 실행을 유발하지 않습니다. Skill 원문, 지적 사항,
작업 요청, 식별자, 공개 JSON은 번역하지 않고 화면 소유의 문구와 표시 형식만 바꿉니다.
...
대시보드의 프로젝트·Skill 선택, 저장된 평가, 채택 요약, 개발 작업부터 다음 사용까지의
근거 흐름 및 상세 보기는 구현되어 있습니다. 경험과 사용자 피드백을 중심으로 한
전체 화면 개편은 별도 후속 작업입니다.
...
[작성 시점의 로드맵](ROADMAP.md)에서 계획 배경과 의존성을,
...
대시보드는 [제품 개요의 실제 화면 캡처](../README.ko.md#대시보드-화면)로 확인할 수 있습니다.
[프로젝트 평가 안내](PROJECT-EVALUATION.md)에 `results/` 형식, Actions,
...
고정 필드·조건 코드로 기록합니다. [선택·진단 계약](PROJECT-EVALUATION.md#assess-one-current-skill)을
...
스키마와 재사용 콜백은 [replay 계약](HACKATHON-CONTRACTS.md)을 참고하세요.
...
상세 내용은 [preflight 계약](HACKATHON-CONTRACTS.md#read-only-local-approval-preflight)을 참고하세요.
...
[게시 절차와 정확한 offline 후보](REVIEWED-PUBLICATION.md)에 명령·권한·제한·세션5
...
공개 배포는 여전히 별도 승인이 필요합니다. 이 오프라인 예제는 실행 경로의 연결을
보여주는 것이며, 실측 개선이나 실제 채택을 입증하지 않습니다.
...
다른 저장소를 추가하기 전에 [준비 명령과 경계](PROJECT-EVALUATION.md#reproducible-sample-preparation)를 확인하세요.
...
바이트 단위로 보존합니다.
[Owner 대시보드 방향](PROJECT-EVALUATION.md#owner-dashboard-direction)에 자동 CI와
...
이 기존 레지스트리의 등록은 대상에 스킬을 설치하지 않으며 pin 변경 명령도 없습니다.
초기 pin은 앞에서 설명한 기록된 작업의 승인·`run-approved` Active 상태와 별개입니다.
Canary·승격·롤백은 미구현입니다. 이 레지스트리는 PR 기반 실시간 모델 평가를 제공하지 않습니다.
현재 대시보드는 게시된 기록을 읽으며, 비공개 실행 상태를 실시간으로 전송하는 화면은 아닙니다.
