# SkillOps operating guide and historical evidence

[Product overview](../README.md) | [Korean guide](OPERATIONS.ko.md)

Detailed commands, safety contracts and historical measurements retained from the
main README during its September 21, 2026 restructuring. Commands run from the
repository root, not from `docs/`. Historical measurements are not validation of
the current source or evaluator. The current product overview is linked above.
...
### Dashboard presentation and planned configuration

The dashboard provides project/Skill selection, saved evaluations, adoption
summaries, a development-to-next-use evidence trace and detailed evidence views.
The broader experience-and-user-feedback-centered redesign remains separate work.
...
See the dated [roadmap](ROADMAP.md) for planning context and dependencies, and
...
See the [dashboard screenshot](../README.md#dashboard-preview) for a static preview.
See [project evaluation](PROJECT-EVALUATION.md) for the `results/` schema,
...
[selection and diagnostic contract](PROJECT-EVALUATION.md#assess-one-current-skill)
...
[replay contract](HACKATHON-CONTRACTS.md) for its schema and callback API.
...
[preflight contract](HACKATHON-CONTRACTS.md#read-only-local-approval-preflight).
...
See [reviewed publication and exact offline candidates](REVIEWED-PUBLICATION.md)
...
links. Public deployment still requires separate authorization. These offline
fixtures demonstrate runtime wiring, not measured improvement or actual adoption.
...
are missing. See [preparation commands and boundaries](PROJECT-EVALUATION.md#reproducible-sample-preparation)
...
[dependency profile contract](PROJECT-EVALUATION.md#automatic-evaluation-support-and-boundaries).
...
See the [owner's dashboard direction](PROJECT-EVALUATION.md#owner-dashboard-direction)
...
## Execution prerequisites and legacy benchmark scope
...
This legacy registry has no pin-update command; its initial pin is separate from
the recorded-work approval and `run-approved` Active state described above.
Canary, promotion and rollback remain unimplemented. PR-triggered live model
evaluation is not provided by this registry. The existing dashboard reads
published records; it is not a live stream of private runtime state.
