...
def changed_projects(root, projects, before, source_commit):
    ...
    affected = set()
    shared = {"skillops/project_profiles.json", "skillops/package.json", "skillops/package-lock.json",
              "skillops/requirements-evaluator.txt",
    ...
    for name in filter(None, diff.stdout.split("\0")):
        if (name in shared or name.startswith(("eval/", "skills/"))
                or (name.startswith("skillops/") and name.count("/") == 1 and name.endswith(".py"))):
            return projects
        parts = name.split("/")
