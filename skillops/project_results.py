...
CORE = tuple("skillops/" + name for name in (
...
def profiles(root):
    path = Path(root) / "skillops/project_profiles.json"
    if not path.exists():
        return {}
    data = read_json(path)
...
def build(root, results, output):
    ...
    index = read_bytes(root / "skillops/dashboard/index.html").decode("utf-8")
    ...
    config = read_json(root / "skillops/dashboard/staticwebapp.config.json")
    ...
    for name in ("styles.css", "sample-data.json"):
        raw = read_bytes(root / "skillops/dashboard" / name)
...
def main():
    ...
    scan.add_argument("--results", type=Path)
    ...
    legacy.add_argument("--results", type=Path)
    ...
    if hasattr(args, "results") and args.results is None:
        args.results = args.root / "results"
    try:
