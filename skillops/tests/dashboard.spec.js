...
async function nativeExamplePage(page, pathname = '/') {
  ...
  for (const id of ['project-a', 'project-b']) {
    const stored = JSON.parse(fs.readFileSync(path.join(__dirname, '../../results', id, 'index.json'), 'utf8'));
    ...
    for (const run of history) {
      for (const name of ['report.json', 'skill-assessments.json', 'skill-evolution.json']) {
        await page.route(`https://dashboard.test/results/${id}/${run.run_id}/${name}`, route => route.fulfill({
          contentType: 'application/json', body: fs.readFileSync(path.join(__dirname, '../../results', id, run.run_id, name)),
        }));
      }
    }
  }
...
test.describe('visual identity', () => {
  ...
  test('candidate summary keeps counts, unknowns and missing evidence distinct', async ({ page }) => {
    ...
    await expect(page.locator('#summary-history')).toBeVisible();
    await page.evaluate(async ({ run, url }) => {
      const { renderProjectSummary } = await import(url);
