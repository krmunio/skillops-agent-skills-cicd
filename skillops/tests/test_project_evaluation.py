...
class ProjectEvaluationTests(unittest.TestCase):
    def test_invalid_dependency_profile_is_rejected_before_model_runtime(self):
        ...
        declarations = (
            ...
            '[tool.skillops\n',
            '[tool.skillops]\ndependency-profile = "pytest"\n'
            'test-dependencies = ["pytest --no-binary=pytest"]\n',
            '[project]\ndependencies = ["pytest --no-binary=pytest"]\n',
            '[project]\ndependencies = ["pytest --no-binary=pytest"]\n'
            '[tool.skillops]\ndependency-profile = "pytest"\ntest-dependencies = ["pytest"]\n',
        )
    ...
    def test_requirements_file_flags_are_rejected_before_model_runtime_without_profile(self):
        m = self.module()
        for filename in ("requirements.txt", "requirements-dev.txt"):
            with self.subTest(filename=filename), tempfile.TemporaryDirectory() as folder:
                ...
                skill.parent.mkdir(parents=True)
                skill.write_text("---\nname: develop\ndescription: Develop.\n---\nCheck.\n")
                (project / filename).write_text("pytest --no-binary=pytest\n")
                (root / "eval").mkdir()
                (root / "eval/skill-guide-rubric.json").write_text('{"dimensions":["clarity"]}')
                policy = {"enabled": True, "authenticated": True,
                          "budget": {"calls": 0, "max_calls": 8, "deadline": m.time.monotonic() + 60}}
        ...
        for identifier in ("sample_repo", "unknown-project", "../outside"):
            with self.subTest(project=identifier), tempfile.TemporaryDirectory() as folder:
                ...
                result = subprocess.run([
                    m.sys.executable, str(root / "skillops/project_evaluation.py"), "--root", str(root),
...
class ChangedProjectTests(unittest.TestCase):
    ...
    def test_shared_evaluator_change_selects_all_but_dashboard_and_results_do_not(self):
        self.write("skillops/dashboard/app.js", "export const changed = true;\n")
    ...
    def test_shared_runtime_manifest_change_selects_all_projects(self):
        for name in ("skillops/package.json", "skillops/package-lock.json",
                     "skillops/requirements-evaluator.txt", "skillops/project_profiles.json",
                     "skillops/skill_pipeline.py"):
            with self.subTest(name=name):
                self.write(name, '{"schema_version":1,"projects":{}}' if name.endswith("project_profiles.json") else "{}")
                self.assertEqual([project["id"] for project in self.select(self.commit())], ["alpha", "beta"])
