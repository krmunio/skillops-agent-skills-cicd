...
class ProjectResultsTests(unittest.TestCase):
    ...
    def test_catalog_hashes_change_with_source_and_evaluator_inputs(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            (root / "skillops").mkdir()
            (root / "skillops/project_profiles.json").write_text('{"schema_version":1,"projects":{}}')
            ...
            (root / "skillops/project_evaluation.py").write_text("changed")
            self.assertNotEqual(before, m.evaluator_hash(root))
            before = m.evaluator_hash(root)
            (root / "skillops/skill_guide.py").write_text("available")
            self.assertNotEqual(before, m.evaluator_hash(root))
            source.unlink()
            source.symlink_to(root / "skillops/project_profiles.json")
    ...
    def test_build_hashes_final_modules_and_resolves_entrypoint_and_imports(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(config["globalHeaders"], module.read_json(root / "skillops/dashboard/staticwebapp.config.json")["globalHeaders"])
            for name in names:
                self.assertFalse((output / f"{name}.js").exists())
                self.assertEqual((root / "skillops/dashboard" / f"{name}.js").read_bytes(), originals[name])
    ...
    def test_build_dependency_changes_invalidate_importers_without_changing_source_files(self):
        ...
        def changed_dependency(path, *args):
            ...
            return raw + b"\n" if Path(path) == root / "skillops/dashboard/views.js" else raw
