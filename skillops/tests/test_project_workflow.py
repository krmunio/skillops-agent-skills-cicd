...
class WorkflowContractTests(unittest.TestCase):
    def test_current_quality_input_is_optional_distinct_and_passed_as_one_quoted_argument(self):
        text = (Path(__file__).resolve().parents[2] / ".github/workflows/project-evaluation.yml").read_text()
    ...
    def test_manual_project_limits_are_not_replicated_per_skill(self):
        ...
        self.assertEqual(evaluate.count("python3 skillops/project_evaluation.py "), 1)
    ...
    def test_evaluation_exposes_stage_logs_and_validated_failure_summary(self):
        ...
        self.assertIn('python3 skillops/evaluation_reporting.py --results ci-results --run-id "$RESULT_RUN" >> "$GITHUB_STEP_SUMMARY"', evaluate)
        self.assertLess(evaluate.index("python3 skillops/project_evaluation.py "), evaluate.index("python3 skillops/evaluation_reporting.py "))
        self.assertLess(evaluate.index("python3 skillops/evaluation_reporting.py "), evaluate.index("name: public-project-results"))
    ...
    def test_dashboard_ci_runs_locked_browser_checks_with_pinned_actions_without_privileged_secrets(self):
        ...
        for command in ("npm --prefix skillops ci --no-audit --no-fund",
                        "skillops/node_modules/.bin/playwright install --with-deps chromium",
                        "npm --prefix skillops run test:dashboard",
                        "cache-dependency-path: skillops/package-lock.json"):
    ...
    def test_job_permissions_and_failure_evidence_are_separated(self):
        ...
        self.assertIn("python3 skillops/project_results.py validate --results ci-results", persist)
        self.assertIn('python3 skillops/project_results.py merge-samples --results "$target/results"', persist)
