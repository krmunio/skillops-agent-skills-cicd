...
class ReviewedPublicationTests(unittest.TestCase):
    def setUp(self):
        ...
        for path in (ROOT / "skillops").glob("*.py"):
            shutil.copyfile(path, self.root / "skillops" / path.name)
        (self.root / "projects/sample_repo").mkdir(parents=True)
        (self.root / "projects/sample_repo/example.py").write_text("value = 1\n")
        (self.root / "skillops/project_profiles.json").write_text('{"schema_version":1,"projects":{}}')
        self.git("init", "-q")
    ...
    def test_untracked_publisher_cannot_claim_an_older_code_commit(self):
        self.git("rm", "skillops/reviewed_publication.py")
        old_code = self.commit()
        shutil.copyfile(ROOT / "skillops/reviewed_publication.py", self.root / "skillops/reviewed_publication.py")
        with self.assertRaises(RuntimeFailure):
            self.prepare(code_sha=old_code)

    def test_same_script_with_different_pinned_validator_bytes_is_rejected(self):
        path = self.root / "skillops/project_results.py"
    ...
    def test_cli_defaults_to_validation_and_reports_failure_without_success(self):
        ...
        with patch.object(m, "__file__", str(self.root / "skillops/reviewed_publication.py")), redirect_stdout(io.StringIO()) as out:
            self.assertEqual(m.main(args), 0)
        self.assertEqual(json.loads(out.getvalue())["stages"]["input_validation"], "complete")
        with patch.object(m, "__file__", str(self.root / "skillops/reviewed_publication.py")), \
    ...
    def test_actual_cli_uses_the_pinned_checkout_without_model_credentials(self):
        run = subprocess.run(
            [sys.executable, str(self.root / "skillops/reviewed_publication.py"), "--code-sha", self.code_sha,
    ...
    def test_official_build_blocks_static_example_with_missing_public_data(self):
        path = self.root / "skillops/dashboard/index.html"
        html = path.read_text()
