...
class RuntimeTests(unittest.TestCase):
    ...
    def test_developer_requires_completed_target_skill_invocation(self):
        ...
        fixture_path = root / "skillops/tests/fixtures/cli-contract.json"
    ...
    def test_malformed_nested_events_persist_terminal_failure_receipts(self):
        root = Path(__file__).resolve().parents[2]
        fixture = json.loads((root / "skillops/tests/fixtures/cli-contract.json").read_text())
        variants = [
    ...
    def test_baseline_assets_have_family_disjoint_tasks_and_reasonable_skill(self):
        ...
        for name in ("projects/sample_repo/issues.py", "skills/develop/SKILL.md", "eval/tasks.json", "eval/rubric.json",
                     "eval/calibration.json", "eval/fixed_checks.py", "eval/skill-guide-rubric.json", "skillops/skill_guide.py"):
...
class EvaluationTests(unittest.TestCase):
    ...
    def test_fingerprint_changes_for_every_evaluation_input(self):
        for name in ("projects/sample_repo/labels.py", "projects/sample_repo/updates.py", "eval/skill-guide-rubric.json", "skillops/skill_guide.py"):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            def invoke(prompt, model, role, work, artifact, expected_skill=None):
                ...
                if role == "developer":
                    ...
                    if len(calls) == 1:
                        events = json.loads((self.root / "skillops/tests/fixtures/cli-contract.json").read_text())["developer"]
                        for event in events:
                            if event["type"] == "session.usage_checkpoint":
